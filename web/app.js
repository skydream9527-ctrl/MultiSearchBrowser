...
(function() {
    ...
    // v2.1.0: RSS 模块（从 app-rss.js 引入）
    const { renderRss, setupRssPage, refreshRss, fetchRssFeed } = window.MSBRss;
    // v2.1.0: 密码管理模块（从 app-passwords.js 引入）
    const { renderPasswords, setupPasswordsPage, editPassword, generatePassword, showPwdUnlock, setupMasterPwd, setupActivityMonitor, evaluatePasswordStrength, touchPwdActivity } = window.MSBPasswords;
    // v2.1.0: 用户脚本模块（从 app-scripts.js 引入）
    const { injectUserScripts, showUserScriptsManager, editUserScript } = window.MSBScripts;
    // v1.8.0: i18n
    ...
    window.MSBApp = {
        ...
        openWebview,
        credApi,
        // v2.1.0: 接收 Android Room 数据变更通知（C2）
        // Android 端原生 Fragment 操作 Room（如长按删除书签）后，会通过
        // WebView.evaluateJavascript 调用本回调，刷新当前 active 的对应页面。
        // 仅刷新 UI，不回写 Room，避免循环。
        onNativeDataChange: (dataType) => {
            if (dataType === 'bookmarks') {
                // 仅当书签页处于 active 时刷新，避免无谓渲染
                if (document.querySelector('#page-bookmarks.active')) renderBookmarks();
            } else if (dataType === 'history') {
                if (document.querySelector('#page-history.active')) renderHistory();
            } else if (dataType === 'notes') {
                if (document.querySelector('#page-notes.active')) renderNotes();
            }
        },
    };
